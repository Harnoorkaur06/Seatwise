...
const Conflicts = {
  ...
  resolve(conflict, examId, generation) {
    const plan = STORAGE.getSeatingPlan(examId, generation);
    if (!plan) return { ok: false, message: "No seating plan found for this examination." };
    ...
    return Seating.applySuggestion(conflict.studentB.rollNo, suggestions[0], plan.examId, plan.generation);
  }
};
